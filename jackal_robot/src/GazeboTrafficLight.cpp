...
namespace gazebo {
  ...
  void GazeboTrafficLight::timerCb(const ros::TimerEvent& event) {
    for (auto light : traffic_lights_) {
      switch (cfg_.override) {
        case jackal_robot::GazeboTrafficLight_NO_FORCE:
        advanceSequence(light.first);
        break;
        case jackal_robot::GazeboTrafficLight_RED:
        setColor(light.first, LightColor::RED, false);
        break;
        case jackal_robot::GazeboTrafficLight_RED_FLASH:
        setColor(light.first, LightColor::RED, true);
        break;
        case jackal_robot::GazeboTrafficLight_GREEN:
        setColor(light.first, LightColor::GREEN, false);
        break;
        case jackal_robot::GazeboTrafficLight_GREEN_FLASH:
  ...
  void GazeboTrafficLight::reconfig(jackal_robot::GazeboTrafficLightConfig& config, uint32_t level) {
    sequence_timestamp_ = 0.0;
    cfg_ = config;
  }
